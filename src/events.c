...
/*   Updated: 2024/10/18 18:32:36 by ngoulios         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handle_key(mlx_key_data_t keydata, void* param) 
{
    ...
    if ((keydata.key == MLX_KEY_LEFT || keydata.key == MLX_KEY_RIGHT || 
        ...
        if (moved) {
            draw_fractal(fractal);  // Redraw the fractal after movement
            mlx_image_to_window(fractal->mlx, fractal->img, 0, 0);  // Update the image
        }
    }

    // Handle close key (Escape)
...
//printf("New bounds after zoom: x_min = %f, x_max = %f, y_min = %f, y_max = %f\n", fractal->x_min, fractal->x_max, fractal->y_min, fractal->y_max);
